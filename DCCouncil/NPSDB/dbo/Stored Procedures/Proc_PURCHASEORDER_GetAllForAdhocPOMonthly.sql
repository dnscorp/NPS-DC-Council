CREATE PROCEDURE [dbo].[Proc_PURCHASEORDER_GetAllForAdhocPOMonthly]      
(      
    @SearchText nvarchar(MAX),      
	@OfficeIDs nvarchar(MAX),      
    @AsOfDate [datetime],    
	@StartDate [datetime],  
...
BEGIN      
   ...
   [PO].[IsDeleted],      
			...
			AND (month([PO].[DateOfTransaction])=month(@CurrentMonth) and Year([PO].[DateOfTransaction])=year(@CurrentMonth))    
			AND ([O].OfficeID in (select val from fnSplit(@OfficeIDs,',')))
            AND      
  ...
  [PO].[ImportID],      
    ...
    BEGIN      
        SELECT      
            ...
            [O].[IndexCode] AS OfficeIndexCode,      
            [O].[IndexTitle] AS OfficeIndexTitle,      
            [O].[IsDeleted] AS OfficeIsDeleted,                [O].[UpdatedDate] AS OfficeUpdatedDate,      
            [O].[Name] AS OfficeName,      
            [O].[PCA] AS OfficePCA,      
            [O].[PCATitle] AS OfficePCATitle,
            [O].CompCode AS OfficeCompCode,      
