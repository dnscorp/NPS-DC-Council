...
{
    public static class NPSReportSummarySheetHelper
    {
        
        public static void GenerateSummarySheet(ExcelGenerator objExcelGenerator, Office objOffice, FiscalYear objFiscalYear, DateTime dtAsOfDate, List<Expenditure> lstExpenditures, List<PurchaseOrder> lstPurchaseOrders)
        {
            ...
            if (lstBudgets != null && lstBudgets.Count > 0)
            {
                ...
                iRowNumber = _SetExpendituresSummary(objExcelGenerator, lstExpenditures, lstPurchaseOrders, iRowNumber);
                ...
                iRowNumber = _SetObligatedPurchaseOrdersSummary(objExcelGenerator, lstPurchaseOrders, lstRecurringTransactions, iRowNumber);
        ...
        private static int _SetExpendituresSummary(ExcelGenerator objExcelGenerator, List<Expenditure> lstExpenditures, List<PurchaseOrder> lstPurchaseOrders, int iRowNumber)
        {
            ...
            {
                iRowNumber = _SetExpendedPurchaseOrderSummary(objExcelGenerator, iRowNumber, lstPurchaseOrders);
            }
        ...
        private static int _SetExpendedPurchaseOrderSummary(ExcelGenerator objExcelGenerator, int iRowNumber, List<PurchaseOrder> lstPurchaseOrders)
        {
            if (lstPurchaseOrders != null && lstPurchaseOrders.Count > 0)
            {
                ...
                {
                    Vendor objVendor = objPurchaseOrder.GetVendorDetails();
                    if (objVendor != null)
                    {
                        if (!lstVendorNames.Contains(objVendor.Name))
                        {
                            ...
                            {
                                List<PurchaseOrder> lstSelected = lstPurchaseOrders.Where(q => q.VendorName.ToUpper().Equals(objVendor.Name.ToUpper())).ToList();
                                double dblPOBalSum = lstSelected.Sum(q => q.VoucherAmtSum);
                                voucherVendorSum.Add(new VendorHelper(objVendor.Name, dblPOBalSum));
                                lstVendorNames.Add(objVendor.Name);
                            }
                            else
                            {
                                voucherVendorSum.Add(new VendorHelper(objVendor.Name, objPurchaseOrder.VoucherAmtSum));
                            }
                        }
                    }
                    else
                    {
                        voucherVendorSum.Add(new VendorHelper(objPurchaseOrder.VendorName, objPurchaseOrder.VoucherAmtSum));
                    }
                }
                if (voucherVendorSum != null && voucherVendorSum.Count() > 0)
                {
                    ...
                    foreach (var keyValue in voucherVendorSum)
                    {
                        ...
                        {
                            _SetPurchaseOrderSummary(objExcelGenerator, keyValue.VendorName, keyValue.Amount, iRowNumber);
                            iRowNumber++;
                        }
                    }
                }
        ...
        private static int _SetObligatedPurchaseOrdersSummary(ExcelGenerator objExcelGenerator, List<PurchaseOrder> lstPurchaseOrders, List<IDataHelper> lstRecurringTransactions, int iRowNumber)
        {
            ...
            if (lstPurchaseOrders != null && lstPurchaseOrders.Count > 0)
            {
                //List<string> lstVendors = lstPurchaseOrderConverted.Select(q => q.VendorName).ToList();
                List<string> lstVendorNames = new List<string>();
                List<VendorHelper> voucherVendorSum = new List<VendorHelper>();
                foreach (PurchaseOrder objPurchaseOrder in lstPurchaseOrders)
                {
                    ...
                    if (objVendor != null)
                    {
                        if (!lstVendorNames.Contains(objVendor.Name))
                        {
                            if (objVendor.IsRolledUp)
                            {
                                List<PurchaseOrder> lstSelected = lstPurchaseOrders.Where(q => q.VendorName.Equals(objVendor.Name)).ToList();
                                double dblPOBalSum = lstSelected.Sum(q => q.POBalSum);
                                voucherVendorSum.Add(new VendorHelper(objVendor.Name, dblPOBalSum));
                                lstVendorNames.Add(objVendor.Name);
                            }
                            else
                            {
                                voucherVendorSum.Add(new VendorHelper(objVendor.Name, objPurchaseOrder.POBalSum));
                            }
                        }
                    }
                    else
                    {
                        voucherVendorSum.Add(new VendorHelper(objPurchaseOrder.VendorName, objPurchaseOrder.POBalSum));
                    }
                }
                ...
                {
                    voucherVendorSum = voucherVendorSum.OrderBy(q => q.VendorName).ToList();
                    foreach (VendorHelper keyValue in voucherVendorSum)
                    {
                        if (Math.Round(keyValue.Amount, 2) != 0)
                        {
                            _SetPurchaseOrderSummary(objExcelGenerator, keyValue.VendorName, keyValue.Amount, iRowNumber);
