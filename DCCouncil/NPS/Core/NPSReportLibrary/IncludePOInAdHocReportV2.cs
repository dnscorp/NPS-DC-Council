...
{
    class IncludePOInAdHocReportV2
    {
        public static void GenerateTransactionSheet(ExcelGenerator objExcelGenerator, FiscalYear objFiscalYear, DateTime dtAsOfDate, List<PurchaseOrderAdhocReportViewModel> lstPurchaseOrders)
        {
            ...
            _SetColumnHeading(objExcelGenerator, "COST CENTER", "D5", "D5");
            ...
            var lstExpended = lstPurchaseOrders.Where(s => s.EntryType == "E").ToList();
            ...
            var lstObligated = lstPurchaseOrders.Where(s => s.EntryType == "O").ToList();
        ...
        public static void GenerateTransactionSheetMonthly(ExcelGenerator objExcelGenerator, FiscalYear objFiscalYear, DateTime dtAsOfDate, List<PurchaseOrderAdhocReportViewModel> lstPurchaseOrders, DateTime dtlocAsOfDate)
        {
           ...
           // _SetSubHeading(objExcelGenerator, " Non-Personal Services Transactions FY " + objFiscalYear.Year.ToString());
            ...
            _SetColumnHeading(objExcelGenerator, "COST CENTER", "D2", "D2");
        ...
        private static int _SetPurchaseOrderSummary(ExcelGenerator objExcelGenerator, List<PurchaseOrderAdhocReportViewModel> lstPurchaseOrders, int iRowNumber, string strHeaderText)
        {
            ...
            {
                foreach (var objPurchaseOrder in lstPurchaseOrders)
                {
                    _SetPurchaseOrderList(objExcelGenerator, objPurchaseOrder, iRowNumber);
                    iRowNumber++;
                }
            }
            else
        ...
        private static void _SetPurchaseOrderList(ExcelGenerator objExcelGenerator, PurchaseOrderAdhocReportViewModel objPurchaseOrder, int iRowNumber)
        {
            string strCellNumber = "A" + iRowNumber;
            bool blnShouldMerge = false;
            string strFormat = "{0}";
            objExcelGenerator.SetText(string.Format(strFormat, objPurchaseOrder.AccountingDate.ToShortDateString()), strCellNumber, blnShouldMerge, string.Empty);
            ...
            objExcelGenerator.SetText(objPurchaseOrder.CostCenter, strCellNumber, blnShouldMerge, string.Empty);
            ...
            objExcelGenerator.SetText(objPurchaseOrder.IndexCode, strCellNumber, blnShouldMerge, string.Empty);
            objExcelGenerator.SetCellFormat(strCellNumber, CellFormatHelper.SetFourthTransactionSheetColumnValue());

            strCellNumber = "F" + iRowNumber;
            
            if (!string.IsNullOrEmpty(objPurchaseOrder.PODescription))
            {
                objExcelGenerator.SetText(objPurchaseOrder.PODescription, strCellNumber, blnShouldMerge, string.Empty);
            }
            else
            {
            ...
            objExcelGenerator.SetText(objPurchaseOrder.Amount.ToString(), strCellNumber, blnShouldMerge, string.Empty);
            //if (blnIsExpended)
            //{
