...
{
    public class AdHocReportV2Helper
    {
        public static Guid GenerateAdHocReport(FiscalYear objFiscalYear,List<long> lstOfficeIds, List<long> lstExpenditureCategoryIds, DateTime dtAsOfDate, List<IDataHelper> lstExpenditures,bool blnIsYearWise,bool blnIsMonthWise,bool blnCustomReportSheet,DateTime? dtStartDate)
        {
            ...
            var lstOffices = new List<Office>();
            foreach (long lOfficeId in lstOfficeIds)
            {
                lstOffices.Add(Office.GetByOfficeID(lOfficeId));
            }

            var lstExpenditureCategories = new List<ExpenditureCategory>();
        ...
        {
            AdHocReportSummarySheetV2Helper.GenerateCustomReportSheet(objExcelGenerator, lstOffices, lstExpenditureCategories, dtAsOfDate, objFiscalYear, lstExpenditures, dtStartDate);
        }
        ...
        private static void _GenerateSheets(ExcelGenerator objExcelGenerator, List<Office> lstOffices, ExpenditureCategory objExpenditureCategory, DateTime dtAsOfDate, FiscalYear objFiscalYear, List<IDataHelper> lstExpendituresCategoryWise,bool blnIsYearWise,bool blnIsMonthWise,DateTime? dtStartDate)
        {
            ...
            {
                AdHocReportSummarySheetV2Helper.GenerateSummarySheetForFixedCategory(objExcelGenerator, lstOffices, objExpenditureCategory, dtAsOfDate, objFiscalYear, lstExpendituresCategoryWise,blnIsYearWise,blnIsMonthWise, dtStartDate);
            }
            else
            {
                AdHocReportSummarySheetV2Helper.GenerateSummarySheetForNonFixedCategory(objExcelGenerator, lstOffices, objExpenditureCategory, dtAsOfDate, objFiscalYear, lstExpendituresCategoryWise,blnIsYearWise,blnIsMonthWise, dtStartDate);
            }
        }
    }
}
